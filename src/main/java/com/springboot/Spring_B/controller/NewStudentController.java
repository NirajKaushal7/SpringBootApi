...
public class NewStudentController {
    ...
    public ResponseEntity<Book> addBook(@PathVariable long id, @RequestBody Book book) {
        Book addedBook = studentService.addBook(id, book);
        if (addedBook != null)
            return ResponseEntity.ok(book);
        else
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Book is not added bcz of invalid Id.");
    }
    ...
    @GetMapping("/{id}")
    public ResponseEntity<Student> findStudentById(@PathVariable("id") Long id)//here pathVariable comes from url and we assign in id
    {
    ...
    public ResponseEntity<Book> getBookById(@PathVariable("idS") Long idS, @PathVariable("idB") Long idB)//here pathVariable comes from url and we assign in id
    {
        Book book = studentService.findByStudentIdAndId(idS, idB);
        if (book != null)
            return ResponseEntity.ok(book);
        else
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Book is not found on this id's.");

    }

    //for finding the book by id
    @GetMapping("/books/{id}")
    public ResponseEntity<Book> getBookById(@PathVariable("id") Long id)//here pathVariable comes from url and we assign in id
    {
        Book book = studentService.findBookById(id);
        if (book != null) {
            return ResponseEntity.ok(book);
        }
        else
    ...
    {
    Book book = studentService.updateBookById(id,newBook);
        if(book != null)
            return ResponseEntity.ok(book);
        else
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Book is not Found.");
